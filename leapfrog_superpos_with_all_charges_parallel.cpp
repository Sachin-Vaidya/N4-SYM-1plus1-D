...
for(int k=1;k<2*N_t-1;k++)
{
    ...
    for(int m=0;m<3;m++)
    {
        for(int n=0;n<3;n++)
        {
            for(int j=0;j<N;j++)
            {
                if(j==0)
                {
                    u000[m][n][0]=V0[m][n][1]-V0[m][n][0]-V0[m][n][0]+V0[m][n][N-1];
                    u000[m][n][N]=V0[m][n][N+1]-V0[m][n][N]-V0[m][n][N]+V0[m][n][2*N-1];
                }
                else if(j==N-1)
                {
                    u000[m][n][N-1]=V0[m][n][0]-V0[m][n][N-1]-V0[m][n][N-1]+V0[m][n][N-2];
                    u000[m][n][2*N-1]=V0[m][n][N]-V0[m][n][2*N-1]-V0[m][n][2*N-1]+V0[m][n][2*N-2];
                }
                else
                {
                    u000[m][n][j]=V0[m][n][j+1]-V0[m][n][j]-V0[m][n][j]+V0[m][n][j-1];
                    u000[m][n][N+j]=V0[m][n][N+j+1]-V0[m][n][N+j]-V0[m][n][N+j]+V0[m][n][N+j-1];
                }
            }
        }
    }
    ...
    for(int m=0;m<3;m++)
    {
        for(int n=0;n<3;n++)
        {
            for(int j=0;j<N;j++)
            {
                u00[m][n][j]=0;
                u00[m][n][N+j]=0;
                dcomp a_c(V0[m][n][j],V0[m][n][N+j]);
                for(int A=0;A<3;A++)
                {
                    dcomp a2(V0[A][n][j],V0[A][n][N+j]);
                    for(int B=0;B<3;B++)
                    {
                        dcomp a1(V0[m][B][j],V0[m][B][N+j]);
                        dcomp a3(V0[A][B][j],V0[A][B][N+j]);
    ...
    for(int m=0;m<3;m++)
    {
        for(int n=0;n<3;n++)
        {
            for(int i=0;i<N;i++)
            {
                v1[m][n][i]=u0[m][n][i]+(V0[m][n][3*N+i]*2*tau);
                v1[m][n][N+i]=u0[m][n][N+i]-(V0[m][n][2*N+i]*2*tau);
                v1[m][n][2*N+i]=u0[m][n][2*N+i]-((2*gamma)*u000[m][n][N+i])-(2*u00[m][n][N+i]*2*tau);//-(eta*V0[m][n][N+i]*tau);
                v1[m][n][3*N+i]=u0[m][n][3*N+i]+((2*gamma)*u000[m][n][i])+(2*u00[m][n][i]*2*tau);//+(eta*V0[m][n][i]*tau);
            }
        }
    }
